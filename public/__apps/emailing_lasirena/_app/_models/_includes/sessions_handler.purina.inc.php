...
class MySessionHandler implements SessionHandlerInterface
{
    ...
    public function open($savePath, $sessionName)
    {
        if ($GLOBALS['CREATE_TABLES'] === true)
        {
            ...
            $sop = "
                    CREATE TABLE IF NOT EXISTS `" . $GLOBALS['CRM_DATABASE_PREFIX'] . "sessions` (
                      `id` char(32) NOT NULL,
    ...
    {
        $sop = "SELECT data FROM " . $GLOBALS['CRM_DATABASE_PREFIX'] . "sessions WHERE  id = ? LIMIT 1";
        $params = array('s', $id);
    ...
    public function write($id, $data)
    {
        ...
        $this->deleteSession($GLOBALS['PREVIOUS_SSID']);

        $sop = "REPLACE INTO " . $GLOBALS['CRM_DATABASE_PREFIX'] . "sessions  (id,access,data) VALUES (?, ?, ?)";
    ...
    public function gc($maxlifetime)
    {
        ...
        $sop = "DELETE FROM " . $GLOBALS['CRM_DATABASE_PREFIX'] . "sessions WHERE  access < ?";
        $params = array('s', $old);
        $this->obj_mysqli->executePreparedStatement($sop, $params);
    ...
    {
        $sop = "DELETE FROM " . $GLOBALS['CRM_DATABASE_PREFIX'] . "sessions WHERE id = ? LIMIT 1";
        $params = array('s', $id);
        $this->obj_mysqli->executePreparedStatement($sop, $params);
    }
...
$handler = new MySessionHandler($obj_crm_mysqli);
...
$GLOBALS['PREVIOUS_SSID'] = session_id();
