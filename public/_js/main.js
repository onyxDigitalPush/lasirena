...
$(document).ready(function () {
    if ($('.system_msg').length > 0) {
        setTimeout(function () {
            $('.system_msg').animate({ 'top': -$('#system_msg').outerHeight() }, 1000, function () {
                $(this).remove();
            });
        }, 3000);
    }
...
function getFormatedDate(date, lang) {
    ...
    switch (lang) {
        case 'ca':
            response[0] = date.toLocaleString('ca-ES', { dateStyle: 'short' });
            response[1] = date.toLocaleString('ca-ES', { dateStyle: 'medium' });
            response[2] = date.toLocaleString('ca-ES', { dateStyle: 'long' });
            break;
        case 'es':
            response[0] = date.toLocaleString('es-ES', { dateStyle: 'short' });
            response[1] = date.toLocaleString('es-ES', { dateStyle: 'medium' });
            response[2] = date.toLocaleString('es-ES', { dateStyle: 'long' });
            break;
    }
    return response;
}
